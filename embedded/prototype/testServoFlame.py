...
try:
    ...
    while True:
        ...
        # if state change -> stop
        if state != "IDLE" and desired_state != state:
            print(f" ({state} -> {desired_state}). Stopping.")
            state = "IDLE"
            movement_start_time = 0 #

        # if movement time over -> stop
        if state != "IDLE" and time.time() - movement_start_time > movement_duration:
            print(f"{movement_duration} seconds reached. Stopping.")
            state = "IDLE"
            movement_start_time = 0

        # change state
        if desired_state != "IDLE" and state == "IDLE":
            state = desired_state
            movement_start_time = time.time()

        # for servo
        if state != last_state:
            if state == "FORWARD" or state == "IDLE":
                set_servo_angle(90)
